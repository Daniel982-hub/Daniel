...
def menu_principal():
    ...
    print("0 - Sair")
    print("==========================================")
    print("  [Servidor HTTP Recomendacoes -> python recomendacoes_server.py]")
...
def menu_favoritos():
    ...
    print("2 - Listar todos os favoritos")
    print("3 - Ver favoritos de um utilizador")
    print("4 - Atualizar lista de favoritos")
    print("5 - Remover favoritos de um utilizador")
    print("0 - Voltar")

def menu_recomendacoes():
    print("\n----- Recomendacoes -----")
    print("1 - Criar recomendacao manual")
    print("2 - Gerar recomendacoes diarias (automatico)")
    print("3 - Listar todas as recomendacoes")
    print("4 - Ver recomendacao por ID")
    print("5 - Ver recomendacoes de um utilizador")
    print("6 - Atualizar recomendacao")
    print("7 - Remover recomendacao")
    print("0 - Voltar")
    ...
    while True:
        ...
        if op == "1":
            ...
            mostrar(*bd_utilizadores.atualizar_utilizador(
                uid, nome or None, email or None, passe or None, plano or None))
...
def gerir_favoritos():
    while True:
        ...
        if op == "1":
            uid  = input("ID Utilizador (ex: U001): ")
            cids = input("IDs de Conteudo separados por virgula (ex: C001,C002): ")
            mostrar(*bd_favoritos.adicionar_favorito(uid, cids))

        elif op == "2":
            mostrar(*bd_favoritos.listar_favoritos())

        elif op == "3":
            uid = input("ID Utilizador (ex: U001): ")
            mostrar(*bd_favoritos.obter_favoritos_utilizador(uid))

        elif op == "4":
            uid  = input("ID Utilizador (ex: U001): ")
            cids = input("Nova lista de IDs (ex: C001,C003): ")
            mostrar(*bd_favoritos.atualizar_favorito(uid, cids))

        elif op == "5":
            uid = input("ID Utilizador (ex: U001): ")
            mostrar(*bd_favoritos.remover_favoritos_utilizador(uid))
...
def gerir_recomendacoes():
    while True:
        ...
        if op == "1":
            uid    = input("ID Utilizador (ex: U001): ")
            cid    = input("ID Conteudo (ex: C001): ")
            motivo = input("Motivo (Enter para automatico): ")
            mostrar(*bd_recomendacoes.criar_recomendacao(uid, cid, motivo or None))

        elif op == "2":
            mostrar(*bd_recomendacoes.gerar_recomendacoes_diarias())

        elif op == "3":
            mostrar(*bd_recomendacoes.listar_recomendacoes())

        elif op == "4":
            rid = input("ID Recomendacao (ex: R001): ")
            mostrar(*bd_recomendacoes.obter_recomendacao(rid))

        elif op == "5":
            uid = input("ID Utilizador (ex: U001): ")
            mostrar(*bd_recomendacoes.obter_recomendacoes_utilizador(uid))

        elif op == "6":
            rid    = input("ID Recomendacao (ex: R001): ")
            motivo = input("Novo motivo (Enter para ignorar): ")
            score  = input("Novo score de relevancia (Enter para ignorar): ")
            mostrar(*bd_recomendacoes.atualizar_recomendacao(
                rid,
                motivo or None,
            ...
            rid = input("ID Recomendacao (ex: R001): ")
            mostrar(*bd_recomendacoes.remover_recomendacao(rid))
